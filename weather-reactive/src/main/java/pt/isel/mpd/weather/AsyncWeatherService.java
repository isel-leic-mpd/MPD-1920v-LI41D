...
public class AsyncWeatherService {
    ...
    public Observable<AsyncLocation> search(String query) {
        return fromCF(api
            ...
            .flatMap(Observable::fromIterable)  // Observable<Observable<LocationInfo>>
            .map(this::toLocation);
    }
    ...
    public Observable<Weather> pastWeather(double lat, double log, LocalDate from, LocalDate to) {
        ...
        return Observable
            .range(0, count + 1)               // Observable<Integer>
            ...
            .flatMap(AsyncWeatherService::fromCF)   // Observable<List<WeatherInfo>>
